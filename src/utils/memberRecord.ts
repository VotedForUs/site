...
export type BillVote = { billId: string; actionDate?: string; date?: string; votes?: Record<string, string> };
 ...
 * both chambers this Congress keep the votes from each. Nothing here reads a
 * member's current chamber, which is what would get those five wrong.
 */
export function memberVotesOnBill<T extends BillVote>(votes: T[], bioguideId: string, billId: string): T[] {
  return votes
    .filter(vote => vote.billId === billId && vote.votes?.[bioguideId])
